...
class Architecture(object):
    def __init__(self, sess, params, trained_model=False, prepare_train_set=True):
        start = time.time()
        self.sess = sess
        self.params = params
        self.train_set = Dataset(self.params, "data/" + self.params.get("corpus_name") + "/train", None, only_eval=False)
        ...
        print("Модель подготовлена за " + str(int(time.time() - start)) + " секунд.")
    ...
    def evaluate(self, files, max_langs_per_file, allowed_langs, output_file, threashold, eval_lines=False, eval_blocks=False, smoothing=0, unknown=None, separator=",", code_swaps=None):
        ...
        datafile = Dataset(self.params, None, "data/" + self.params.get("corpus_name") + "/train", only_eval=True, use_eol=eval_lines)
            ...
            for filename in files:
                ...
                for max in np.argsort(-guesses):
                    ...
                    if guess_name in allowed_langs:
                        ...
                        print(filename[1] + ", not allowed lang: " + guess_name)
        ...
        if languages is not None:
            ...
            for l in languages:
                ...
                if id == Vocabulary.Vocab.UNK_ID:
                    ...
                    langs_mask[id] = 1
        datafile = Dataset(self.params, None, "data/" + self.params.get("corpus_name") + "/train", text_to_eval=text)
    ...
    def training(self, eval=None):
        ...
        dev = Dataset(self.params, "data/" + self.params.get("corpus_name") + "/dev", "data/" + self.params.get("corpus_name") + "/train")
        ...
        while not stop:
            ...
            l, _ = self.model.run(self.sess, batch_xs, batch_ys, lengths, self.params.get("dropout"))
            ...
            if self.params.params["step"] % self.params.get("steps_per_checkpoint") == 0 or stop:
                ...
                while not dev.is_finished() and eval is None:
                    ...
                    dropout = 1
                    _, out = self.model.run(self.sess, dev_batch_xs, dev_batch_ys, lengths, dropout)
            ...
            if self.train_set.is_finished():
                ...
                summ = self.sess.run(self.model.performance_summaries, feed_dict={self.model.tf_loss_ph: avg_loss, self.model.tf_accuracy_ph: avg_test_accuracy})
