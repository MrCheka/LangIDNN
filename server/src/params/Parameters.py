...
class Parameters(object):
    ...
    def add_string(self, name, default_value):
        if " " in default_value:
            raise NotImplementedError("Пробел не может быть значением по умолчанию")
        if name not in self.params:
    ...
    def get(self, name):
        if name not in self.params:
            logging.error("Параметр {0} не установлен.".format(name))
            exit(7)
    ...
    def get_column_id(self,name):
        for i in range(len(self.columns)):
            if self.columns[i][0]==name:
                return i
        logging.error("Столбец {0} не найден.".format(name))
        exit(7)
    ...
    def load_params(self, identification):
        with codecs.open(self.filename, encoding='utf-8', mode='r') as f:
            ...
            logging.error("Параметры для {0} не найдены".format(identification))
            exit(7)
